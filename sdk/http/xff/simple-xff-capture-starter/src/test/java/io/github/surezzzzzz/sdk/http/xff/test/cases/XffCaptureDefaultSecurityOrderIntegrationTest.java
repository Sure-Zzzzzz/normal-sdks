...
/**
 * 默认 Capture 顺序与真实 Spring Security 链路集成测试。
 *
 * @author surezzzzzz
 */
@Slf4j
...
class XffCaptureDefaultSecurityOrderIntegrationTest {
    ...
    @Test
    void shouldCaptureAfterSecurityChainWithoutTreatingRemoteAddrAsXff() {
        ...
        log.info("真实默认 Security/Capture 链路：captureOrder={}，securityOrder={}，"
        ...
        assertEquals(Integer.MAX_VALUE - 100,
                simpleXffCaptureFilterRegistration.getOrder(),
                "未配置时 Capture Filter 应在最低优先级前保留 100 的顺序空间");
        assertTrue(simpleXffCaptureFilterRegistration.getOrder()
                        > securityFilterChainRegistration.getOrder(),
                "默认 Capture Filter 应在真实 Security Filter Chain 后执行");
        ...
        assertFalse(event.getXffChain().isPresent(), "审计事件不应将 remoteAddr 回填为 XFF");
