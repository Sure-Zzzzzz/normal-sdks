package io.github.surezzzzzz.sdk.elasticsearch.orm.metadata;
...
import io.github.surezzzzzz.sdk.elasticsearch.orm.constant.Constants;
import io.github.surezzzzzz.sdk.elasticsearch.orm.constant.ErrorMessages;
import io.github.surezzzzzz.sdk.elasticsearch.orm.constant.FieldType;
import io.github.surezzzzzz.sdk.elasticsearch.orm.constant.SensitiveStrategy;
import io.github.surezzzzzz.sdk.elasticsearch.orm.metadata.model.FieldMetadata;
...
public class MappingManagerImpl implements MappingManager {
    ...
    public void init() {
        ...
        for (SimpleElasticsearchOrmProperties.IndexConfig indexConfig : properties.getIndices()) {
     ...
     * 定时刷新（如果启用）
     */
    @Scheduled(fixedDelayString = "#{${io.github.surezzzzzz.sdk.elasticsearch.orm.mapping-refresh.enabled:false} ? " +
            "${io.github.surezzzzzz.sdk.elasticsearch.orm.mapping-refresh.interval-seconds:300} * 1000 : Long.MAX_VALUE}")
    ...
    public IndexMetadata getMetadata(String indexAlias, String specificIndexName) {
        ...
        // 2. 查找配置
        SimpleElasticsearchOrmProperties.IndexConfig indexConfig = findIndexConfig(indexAlias);
        if (indexConfig == null) {
    ...
    @Override
    public List<SimpleElasticsearchOrmProperties.IndexConfig> getAllIndices() {
     ...
     */
    private IndexMetadata loadMetadata(SimpleElasticsearchOrmProperties.IndexConfig indexConfig, String specificIndexName) throws IOException {
    ...
    private void parseFields(Map<String, Object> propertiesMap, String prefix,
                             ...
                             SimpleElasticsearchOrmProperties.IndexConfig indexConfig) {
        for (Map.Entry<String, Object> entry : propertiesMap.entrySet()) {
            ...
            // 检查是否为敏感字段
            SimpleElasticsearchOrmProperties.SensitiveFieldConfig sensitiveConfig =
     ...
     */
    private SimpleElasticsearchOrmProperties.IndexConfig findIndexConfig(String identifier) {
        for (SimpleElasticsearchOrmProperties.IndexConfig config : properties.getIndices()) {
            // 优先匹配alias（如果有）
     ...
     * 查找敏感字段配置
     */
    private SimpleElasticsearchOrmProperties.SensitiveFieldConfig findSensitiveFieldConfig(
            SimpleElasticsearchOrmProperties.IndexConfig indexConfig, String fieldName) {
        if (indexConfig.getSensitiveFields() == null) {
            return null;
        }
        for (SimpleElasticsearchOrmProperties.SensitiveFieldConfig config : indexConfig.getSensitiveFields()) {
