...
文档模板渲染 SDK。提供统一的模板语法和渲染接口，当前支持 Word（.docx）渲染与 PDF 输出，后续可扩展 Markdown、HTML 等格式。
...
- `renderForPdf(...)` / `renderToPdf(...)` 会将模板中的 `[suredt.chart:key]` 图表渲染为 PNG 后嵌入 PDF
