...
文档模板渲染 SDK。提供统一的模板语法和渲染接口，当前支持 Word（.docx）和 Markdown（.md）模板渲染，并支持 DOCX / Markdown 输出 PDF。
...
// PDF 快捷方法：DOCX 模板渲染后直接输出 PDF 字节
byte[] pdfBytes = templateEngine.renderToPdf("classpath:templates/report.docx", data);

// PDF 链式 API：支持 toBytes / toStream / toFile
PdfRenderResult pdfResult = templateEngine.renderForPdf("classpath:templates/report.docx", data);
pdfResult.toFile("/output/report.pdf");
...
### MdHelper：Markdown 模板渲染

适用于 Markdown 模板 + data 生成 Markdown 或 PDF：

```java
@Autowired
private MdHelper mdHelper;

// Markdown 模板 → Markdown bytes
byte[] mdBytes = mdHelper.render("classpath:templates/report.md", data);

// Markdown 模板 → PDF bytes
byte[] pdfBytes = mdHelper.renderPdf("classpath:templates/report.md", data);

// Markdown 模板 → PDF 文件
mdHelper.renderPdfToFile("classpath:templates/report.md", data, "/data/report.pdf");

// Markdown 模板 → Markdown 文件
mdHelper.renderToFile("classpath:templates/report.md", data, "/data/report.md");

// Markdown 模板 → Markdown/PDF 流
mdHelper.renderToStream("classpath:templates/report.md", data, response.getOutputStream());
mdHelper.renderPdfToStream("classpath:templates/report.md", data, response.getOutputStream());
```
...
| 我有 Markdown 模板，要生成 Markdown 或 PDF | `MdHelper` |
...
- **业务优先使用 Helper**：常规业务代码优先注入 `DocxHelper` / `MdHelper` / `PdfConvertHelper` / `ChartPngHelper`，只有需要链式输出、空文档检查、显式格式指定等高级能力时再使用 `TemplateEngine`。
- **模板渲染和已有文件转换分开**：`DocxHelper` 只处理“DOCX 模板 + data → DOCX/PDF”；`MdHelper` 只处理“Markdown 模板 + data → Markdown/PDF”；已有 DOCX bytes、上传流、本地文件转 PDF 使用 `PdfConvertHelper`，避免重复渲染模板。
...
1.2.0 起 `.md` 模板支持与 Word 相同的变量、条件块、图片占位符，并支持 Markdown → PDF：

```markdown
# [suredt.var:title]

[suredt.start:hasRisk]
## 风险列表

| 名称 | 状态 |
|------|------|
[suredt.for:riskList]
| [suredt.var:name] | [suredt.var:status] |
[suredt.endfor:riskList]
[suredt.end:hasRisk]
...
data.put("logo", new Image("classpath:images/logo.png", 120, 80, "logo"));
...
如果只需要将 `Chart` 模型渲染为 PNG（例如邮件、HTML、预览、自定义报表），可以直接注入 `ChartPngHelper`：

```java
@Autowired
private ChartPngHelper chartPngHelper;

byte[] pngBytes = chartPngHelper.toPng(trendChart);
byte[] fixedSizePng = chartPngHelper.toPng(trendChart, 600, 360);
```

`ChartPngHelper` 的字体来源与 PDF 输出保持一致，统一使用 `io.github.surezzzzzz.sdk.template.doc.font-paths` 配置。
...
1.1.0 起支持将 DOCX 模板渲染结果直接输出为 PDF，1.2.0 起 `renderToPdf(...)` 同时支持 Markdown 模板输出 PDF。

```java
// 快捷方式：DOCX 模板直接拿 PDF bytes
byte[] pdfBytes = templateEngine.renderToPdf("classpath:templates/weekly-report.docx", data);

// 快捷方式：Markdown 模板直接拿 PDF bytes
byte[] mdPdfBytes = templateEngine.renderToPdf("classpath:templates/weekly-report.md", data);

// DOCX PDF 专用链式方式：按需写文件、写流或取 bytes
PdfRenderResult pdfResult = templateEngine.renderForPdf("classpath:templates/weekly-report.docx", data);
pdfResult.toFile("/output/weekly-report.pdf");
pdfResult.toStream(response.getOutputStream());
byte[] bytes = pdfResult.toBytes();
```

`HtmlPdfRenderEngine` 是内部 XHTML → PDF 渲染引擎，供 DOCX PDF 和 Markdown PDF 链路复用；1.2.0 不提供 `HtmlHelper`，也不支持 HTML 模板变量渲染。

如果只是已有 DOCX 字节需要直接转 PDF，也可以注入 `PdfOutputHandler`：

```java
@Autowired
private PdfOutputHandler pdfOutputHandler;

byte[] pdfBytes = pdfOutputHandler.convertToPdf(docxBytes);
pdfOutputHandler.convertToPdf(docxBytes, response.getOutputStream());
```

**图表说明：**

- `renderForPdf(...)` / `renderToPdf(...)` 会将 DOCX 模板中的 `[suredt.chart:key]` 图表渲染为 PNG 后嵌入 PDF
- Markdown 模板不支持 `[suredt.chart:key]`；如需在 Markdown PDF 中展示图表，请先用 `ChartPngHelper` 生成 PNG，再作为 `Image` 传入
- `PdfOutputHandler.convertToPdf(byte[])` 面向已有 DOCX 的直接转换，不会重新绑定业务数据；如需保证模板图表进入 PDF，请优先使用 `renderForPdf(...)`
- 页眉、页脚中的 chart 占位符不支持，正文段落中使用即可

**字体说明：**

PDF 和 Chart PNG 都使用 `io.github.surezzzzzz.sdk.template.doc.font-paths` 配置加载字体。路径可以是字体文件，也可以是字体目录。
...
Linux 环境如果 PDF 中文显示异常，优先确认运行容器或服务器中已安装可用中文字体，并通过 `font-paths` 显式配置。
...
| `fontPaths` | List<String> | 常见 Windows/macOS/Linux 中文字体路径 | PDF 与 Chart PNG 渲染字体来源。可配置字体文件或目录，配置后覆盖默认值 |
| `maxTemplateBytes` | long | `10485760`（10MB） | 模板读取最大字节数，超出拒绝，防止超大模板耗尽内存（1.2.0 受限读取） |
| `maxImageBytes` | long | `5242880`（5MB） | 图片读取最大字节数，超出拒绝，防止超大图片耗尽内存（1.2.0 受限读取） |
...
- Markdown → PDF 时图片会被转为受控 data URI，不会泄露内部 token
...
| `TemplateRenderException` | `MD_005` | Markdown 安全校验拒绝（危险 scheme、绝对 URI、路径逃逸、非法 data URI 等） |
| `TemplateRenderException` | `OUTPUT_005` | 链式 `output(OutputFormat.PDF)` 暂不支持，引导走 `renderToPdf(...)` |
| `TemplateRenderException` | `PDF_006` | HTML/XHTML 转 PDF 失败 |
| `DocxToPdfFailedException` | `PDF_001` | DOCX 转 PDF 失败 |
| `FontNotFoundException` | `PDF_002` | PDF 或 Chart PNG 渲染所需字体缺失 |
| `ChartPngGenerationException` | `PDF_003` | Chart 渲染 PNG 失败 |
| `PdfLibLoadException` | `PDF_004` | PDF 相关库加载或初始化失败 |
| `PdfFooterUnsupportedException` | `PDF_005` | PDF 页脚中包含暂不支持的复杂元素 |
| `EmbeddedFontParseException` | `FONT_001` | DOCX 内嵌字体解析失败 |
