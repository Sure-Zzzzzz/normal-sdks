...
public class SpELHelperTest {
    ...
    public void testIsSpEL() {
        assertTrue(SpELHelper.isSpEL("#{expression}"));
        assertTrue(SpELHelper.isSpEL("#{T(java.lang.String).valueOf('test')}"));

        assertFalse(SpELHelper.isSpEL("plain_text"));
        assertFalse(SpELHelper.isSpEL(null));
        assertFalse(SpELHelper.isSpEL(""));
    ...
    public void testResolveSimpleExpression() {
        // 字符串拼接
        String result = SpELHelper.resolve("#{'prefix_' + 'suffix'}");
        assertEquals("prefix_suffix", result);

        // 静态方法调用
        result = SpELHelper.resolve("#{T(java.lang.String).valueOf(123)}");
        assertEquals("123", result);

        // 非 SpEL 表达式
        result = SpELHelper.resolve("plain_index");
    ...
    public void testSpELWithoutRequestContext() {
        String result = SpELHelper.resolve(SPEL_EXPRESSION);
    ...
    public void testResolveInvalidExpression() {
        ...
        String result = SpELHelper.resolve(invalid);
        assertEquals(invalid, result);

        // 不存在的类
        String nonExistent = "#{T(com.example.NonExistentClass).method()}";
        result = SpELHelper.resolve(nonExistent);
    ...
    public void testSpELCache() {
        ...
        // 清空缓存
        SpELHelper.clearCache();
        assertEquals(0, SpELHelper.getCacheSize());
        log.info("缓存已清空");

        // 首次解析
        String expr1 = "#{'cached_value'}";
        String result1 = SpELHelper.resolve(expr1);
        assertEquals("cached_value", result1);
        assertEquals(1, SpELHelper.getCacheSize());
        log.info("首次解析后缓存大小: 1");

        // 再次解析相同表达式（应该命中缓存）
        String result2 = SpELHelper.resolve(expr1);
        ...
        String result3 = SpELHelper.resolve(expr2);
        assertEquals("another_value", result3);
        assertEquals(2, SpELHelper.getCacheSize());
        log.info("解析不同表达式后缓存大小: 2");

        // 解析非 SpEL 表达式（不应进入缓存）
        String plain = "plain_text";
        String result4 = SpELHelper.resolve(plain);
    ...
    public void testSpELCachePerformance() {
        ...
        String result1 = SpELHelper.resolve(expr);
        ...
        String result2 = SpELHelper.resolve(expr);
