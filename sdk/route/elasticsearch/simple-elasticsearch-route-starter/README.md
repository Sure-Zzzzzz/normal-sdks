...
- 📅 **日期分片**：写操作自动路由至日期分片索引，读操作通配模式查询历史分片（1.1.0+）
- 🌏 **显式时区**：支持全局默认和 rule 级 `write-index.zone-id`，避免容器/JVM 时区差异影响日期分片（1.1.1+）
- ⚡ **异步写**：规则级别启用异步写，写请求提交线程池立即返回，适合日志/审计场景（1.1.0+）
- 🔌 **多版本兼容**：支持 Elasticsearch 6.x / 7.x，自动版本探测
- 🧩 **兼容公共 Helper**：提供 ES 版本、XContent、low-level REST、PIT/scroll、聚合协议等公共兼容 helper（1.2.0+）
- 💪 **版本自适应客户端**：提供 RestHighLevelClient，屏蔽版本差异
- ⚡ **高性能**：路由规则缓存、SpEL 表达式缓存、正则编译缓存、DateTimeFormatter 缓存
...
- 1.1.0 通过反射检测和 HTTP 兼容路径补齐旧版 API 能力，低版本也纳入正式适配范围。1.2.0 已继续覆盖 2.2.x / 2.3.12 / 2.4.5 / 2.7.9 全版本验证。
...
route 1.2.0 将 search-starter / persistence-starter 中可复用、且不依赖下游业务模型的 ES 6.x / 7.x 兼容经验沉淀到 route 的 `support` 包。
...
`write-index.template` 渲染日期时支持显式指定时区，避免容器、不同机器或不同 JVM 默认时区导致同一批数据写入不同日期分片。
...
非法 `write-index.zone-id` / `write-index-zone-id` 不阻断启动：全局非法时降级到 JVM 默认时区；rule 级非法时降级到全局默认时区，全局也不可用时使用 JVM 默认时区。

**兼容说明：**

1.1.1 推荐使用 `write-index.template`、`write-index.zone-id`、`read-index.pattern`。为兼容 1.1.0，旧平铺配置 `write-index-template`、`write-index-zone-id`、`read-index-pattern` 仍然可用；新旧同时配置时，新分组配置优先。

### 2. DateTimeFormatter 本地缓存

日期分片模板渲染时，按 `{pattern}` 缓存 `DateTimeFormatter`，避免高频写场景重复创建 formatter 对象：

- 相同 pattern 复用缓存
- 不同 pattern 独立缓存
- pattern 语法合法但无法由 `LocalDate` 渲染时原样返回模板，且不保留该 formatter 缓存
...
rules:
  - pattern: "app-log-*"
    ...
    datasource: logging
    write-index:
      template: "app-log-{yyyy.MM.dd}"   # 写入当日索引，如 app-log-2026.07.01
    read-index:
      pattern: "app-log-*"                # 查询覆盖所有历史分片
```
...
这个示例包含 1.1.0/1.1.1 的新增配置：`proxy-type`、`async-write-thread-pool-size`、`write-index.template`、`read-index.pattern`、`async-write`、全局 `write-index.zone-id`、rule 级 `write-index.zone-id`，并同时覆盖精确、前缀、后缀、通配符、正则 5 种匹配类型。
