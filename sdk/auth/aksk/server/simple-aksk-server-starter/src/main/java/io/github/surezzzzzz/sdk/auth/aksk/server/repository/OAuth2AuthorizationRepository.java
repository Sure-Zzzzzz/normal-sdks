...
public class OAuth2AuthorizationRepository {
    ...
    private TokenInfo convertToTokenInfo(OAuth2AuthorizationEntity authorization) {
        ...
        boolean isRevoked = isTokenRevoked(authorization.getAccessTokenMetadata());
        if (isRevoked) {
    ...
    /**
     * 判断 token 是否被撤销
     * metadata 是 JSON 格式，检查 "metadata.token.invalidated" 字段
     */
    private boolean isTokenRevoked(byte[] metadataBytes) {
        if (metadataBytes == null || metadataBytes.length == 0) {
            return false;
        }
        try {
            String json = new String(metadataBytes, java.nio.charset.StandardCharsets.UTF_8);
            JsonNode root = OBJECT_MAPPER.readTree(json);
            JsonNode invalidated = root.get("metadata.token.invalidated");
            return invalidated != null && invalidated.asBoolean(false);
        } catch (Exception e) {
            log.debug("Failed to parse token metadata", e);
        }
        return false;
    }
