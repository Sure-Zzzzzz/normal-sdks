...
本版本引入事件发布机制，在 Header 认证成功后自动发布 Spring 事件，支持业务监听事件实现审计、监控等功能。

## 新增功能

### 1. Header 认证事件发布

在 Header 认证成功后，自动发布 `AkskAccessEvent` 事件。

**事件内容：**
- 客户端信息：clientId、clientType
- 用户信息：userId、username、roles、scope
- 请求信息：requestUri、httpMethod、remoteAddr、userAgent
- 元数据：source（固定为 "header"）、traceId、context
...
```gradle
implementation "io.github.surezzzzzz:simple-aksk-security-context-starter:1.0.2"
```
...
记录谁在什么时间通过 Header 访问了哪些 API。

### 2. 安全监控

监控 Header 访问行为，如异常访问、权限滥用等。
...
public class MyAkskAccessListener {
    ...
    public void onAkskAccessEvent(AkskAccessEvent event) {
        log.info("Header access: user={}, uri={}, method={}, source={}",
            event.getUsername(),
            event.getRequestUri(),
            event.getHttpMethod(),
            event.getSource()  // "header"
        );
    }
}
```
