# Changelog - simple-aksk-resttemplate-redis-client-starter 3.0.1
...
- 公开 API、配置键（`auth.aksk.client.*` / `cache.*` / `resttemplate.*`）零变化，业务代码无需修改。
...
- 四版本矩阵（SDK starter 基线）：Spring Boot 2.7.9 / 2.4.5 / 2.3.12.RELEASE / 2.2.13.RELEASE，每版本 `cleanTest test --rerun-tasks` 全量重跑，3 个测试类 17 个用例全部通过（0 skipped / 0 failures / 0 errors），已核对 JUnit XML 计数；含真实调用 AKSK Server `/api/token/statistics` 的 E2E（自动加头 / 缓存复用 / 过期刷新 / 无效凭证 401）。
- E2E 针对 AKSK Server 3.1.0（`/api` 已移交公共资源层鉴权）验证通过。
