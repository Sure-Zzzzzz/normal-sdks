...
| `simple-aksk-server-starter` | 1.1.0 | 服务端 | AKSK 认证服务器，L1+L2 两级缓存 |
| `simple-aksk-client-core` | 1.0.0 | 客户端核心 | Token 管理抽象接口 |
| `simple-aksk-redis-token-manager` | 1.0.0 | 客户端 | Redis Token 缓存管理 |
...
解析后同样发布 `AkskAccessEvent` 事件，可通过 `SimpleAkskSecurityContextHelper` 读取。

---

## 7. 审计模块

### 7.1 Token 审计事件（simple-aksk-server-core:1.0.2）
...
通过 `TokenEventType` 枚举区分类型，无需硬编码字符串。
...
public void onTokenIssued(TokenIssuedEvent event) {
    log.info("Token issued: clientId={}, scopes={}, eventTime={}",
        event.getClientId(), event.getScopes(), event.getEventTime());
}
...
### 7.2 AKSK 访问事件（simple-aksk-resource-core:1.0.1）
...
引入 `simple-aksk-server-core:1.0.2`，实现 `@EventListener` 监听 `TokenIssuedEvent`、`TokenRevokedEvent` 等事件即可。
...
**Q8：启动时出现 `Cannot load module CasJackson2Module` 警告怎么处理？**
...
不需要任何 SmartCache 配置，只需确保 `redis.enabled` 为 `false`（默认值），服务自动降级为纯 MySQL 模式。`SmartCacheManager` 不会被注入，所有 token 操作直接走 JDBC。
...
| 1.0.4 | - | Admin 编辑权限页面 UX 优化 |
| 1.0.3 | - | OAuth2 scope 权限控制、JWT 安全上下文提取 |
| 1.0.2 | 2026-02-04 | JWT 新增 `auth_server_id` claim |
| 1.0.1 | 2026-01-31 | scope 参数可选、Admin 页面增强 |
| 1.0.0 | 2026-01-19 | 初始版本 |

### smart-cache-starter（依赖）

| 版本 | 日期 | 说明 |
|------|------|------|
| 1.0.4 | 2026-04-17 | 修复多副本 Pub/Sub 消息被错误忽略（sender 改用 UUID instanceId） |
