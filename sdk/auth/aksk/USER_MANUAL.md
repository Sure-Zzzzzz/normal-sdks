...
> **1.x 封版文档**：如果你使用的是 1.x 版本，请查看 [USER_MANUAL.1.x.md](USER_MANUAL.1.x.md)。
...
- **JWE Token 签发**：Token payload 加密（A256GCMKW + A256GCM），截获后无法解读内容
...
┌─────────────────────────────────────────────────────────────────┐
│                     资源服务层 (Resource)                        │
│  INTROSPECT 验证（resource-server-starter）                      │
└─────────────────────────────────────────────────────────────────┘
```
...
```
客户端 → AKSK Server 换 JWE Token → 携带 Token → 业务服务
                                                      ↑
                                          resource-server-starter
                                          （INTROSPECT 模式，调用 /oauth2/introspect）
```

- 业务服务引入 `simple-aksk-resource-server-starter`，配置 introspect 端点
- 启用 L1 本地缓存后（默认开启，TTL 3s），热路径命中缓存时无 HTTP 调用
...
| 模块 | 版本 | 类型 | 说明 |
|------|------|------|------|
| `simple-aksk-core` | 2.0.0 | 核心 | 公共常量、模型、工具类 |
| `simple-aksk-server-core` | 2.0.1 | 核心 | Server 公共配置、审计事件定义 |
| `simple-aksk-server-starter` | 2.0.1 | 服务端 | AKSK 认证服务器，JWE Token，L1+L2 两级缓存 |
| `simple-aksk-client-core` | 2.0.0 | 客户端核心 | Token 管理抽象接口 |
| `simple-aksk-redis-token-manager` | 2.0.0 | 客户端 | Redis Token 缓存管理，L1+L2+Preload |
          ...
          aksk:
            server:
              jwt:
                ...
                encryption-key: <Base64 AES-256 密钥>   # openssl rand -base64 32
              admin:
                enabled: true
                username: admin
...
| `jwt.encryption-key` | - | **必填**：AES-256 密钥（Base64 编码，32 字节），用于 JWE 加密 |
...
> Token 本身为 JWE 格式（5 段 Base64URL），payload 已加密，无法在客户端直接解析。以上字段通过 `/oauth2/introspect` 端点获取。
...
Feign 客户端，自动在请求头中添加 `Authorization: Bearer {token}`。
...
RestTemplate 客户端，自动在请求头中添加 `Authorization: Bearer {token}`。
...
资源端核心模块，提供权限注解和安全上下文工具。

**权限注解**：

| 注解 | 说明 | 示例 |
|------|------|------|
| `@RequireContext` | 要求请求携带 AKSK 安全上下文 | `@RequireContext` |
| `@RequireField` | 要求上下文中某字段不为空 | `@RequireField("userId")` |
| `@RequireFieldValue` | 要求上下文中某字段等于指定值 | `@RequireFieldValue(field="clientType", value="platform")` |
| `@RequireExpression` | SpEL 表达式校验 | `@RequireExpression("#context['scope'].contains('/api/read')")` |
...
开箱即用的 Server 端审计 starter，监听 Token 生命周期事件并转换为 `ServerTokenAuditRecord`。

```gradle
implementation 'io.github.sure-zzzzzz:simple-aksk-server-audit-listener-starter:2.0.0'
```

实现 `ServerTokenAuditHandler` 接口即可接收审计记录：
...
开箱即用的 Resource 端审计 starter，监听 `AkskAccessEvent` 并转换为 `AkskAuditRecord`。

```gradle
implementation 'io.github.sure-zzzzzz:simple-aksk-resource-audit-listener-starter:2.0.0'
```

实现 `AkskAuditHandler` 接口即可接收审计记录：
...
```gradle
implementation 'io.github.sure-zzzzzz:simple-aksk-server-starter:2.0.1'
...
平台级（AKP）不绑定用户，适合服务间调用；用户级（AKU）绑定具体用户，introspect 响应中包含 `user_id` 和 `username`，适合代表用户发起的调用。
...
**Q3：Feign 和 RestTemplate 两个 starter 能同时用吗？**
...
**Q6：启动时出现 `Cannot load module CasJackson2Module` 警告怎么处理？**
...
`consistency.mode=strong` 时，任意实例 revoke token 后通过 Redis Pub/Sub 广播缓存失效消息，其他实例的 L1 缓存立即清除。

**Q9：Token 是什么格式？**

2.0.0 起 Token 为 JWE 格式（5 段 Base64URL），payload 已加密，无法在客户端直接解析。通过 `/oauth2/introspect` 端点获取 claims。
...
| 版本 | 日期 | 说明 |
|------|------|------|
| 2.0.1 | 2026-05-28 | 修复 raw JWT token 反序列化失败（`deserializeTokenValue` 无法解析以 `eyJ` 开头的 raw token） |
| 2.0.0 | 2026-05-21 | JWE Token（A256GCMKW + A256GCM），新增 `jwt.encryption-key` 必填配置，包结构重构 |
...
| 版本 | 日期 | 说明 |
|------|------|------|
| 2.0.0 | 2026-05-26 | 移除 JWT 解析和 nimbus-jose-jwt 依赖；Token 有效性改由缓存 TTL 保证 |

### simple-aksk-redis-token-manager

| 版本 | 日期 | 说明 |
|------|------|------|
| 2.0.0 | 2026-05-27 | TokenWithExpiry 改用 expiresAt（绝对 epoch 秒）；preload 由 smart-cache 框架 Redis TTL 机制驱动；分布式一致性保证（reload 从 Redis 读 securityContext） |

### simple-aksk-feign-redis-client-starter

| 版本 | 日期 | 说明 |
|------|------|------|
| 2.0.0 | 2026-05-27 | 依赖 redis-token-manager 升至 2.0.0 |
...
| 1.0.2 | 2026-04-13 | 新增 Introspect 验证模式 |
| 1.0.0 | - | 初始版本 |
...
| 版本 | 日期 | 说明 |
|------|------|------|
| 2.0.0 | 2026-05-26 | 依赖升级至 resource-core 2.0.0；移除 security-context-starter；仅支持 INTROSPECT 模式 |
| 1.0.0 | - | 初始版本，支持 Header / JWT 认证审计 |

### simple-aksk-server-audit-listener-starter

| 版本 | 日期 | 说明 |
|------|------|------|
| 2.0.0 | 2026-05-26 | 依赖升级至 server-core 2.0.1、server-starter 2.0.0 |
| 1.0.0 | - | 初始版本 |
