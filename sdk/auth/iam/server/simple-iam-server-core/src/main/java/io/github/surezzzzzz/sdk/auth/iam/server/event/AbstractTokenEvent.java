...
/**
 * IAM Token 生命周期事件抽象基类。
 *
 * <p>事件类型描述发生的生命周期动作，业务原因描述动作的触发来源，二者保持正交，
 * 以保证既有按事件类型消费的实现无需因新增原因而改变分支。
 *
 * <p>仅携带非敏感元数据；tokenValue 仅供事件发布链路内部使用，
 * 下游审计处理器不得接收、记录或重建该值。
 *
 * @author surezzzzzz
 */
@Getter
public abstract class AbstractTokenEvent extends AbstractIamEvent {
    ...
    /**
     * 触发事件的业务原因；缺失时归一化为 {@link TokenEventCause#UNSPECIFIED}。
     */
     ...
     * 创建携带明确业务原因的生命周期事件。
     *
     * <p>允许 {@code cause} 为空以兼容上游扩展，空值会归一化为未指定原因
     * 而不会向消费者暴露空语义。
     */
