...
public class IamSessionService {
    ...
    private final IamOAuth2AuthorizationRevocationService authorizationRevocationSupport;
    private final SimpleIamServerProperties properties;
    private final IamAuditEventPublisher auditEventPublisher;
    /**
     * 延迟解析打断构造环：IamMessageSseService 懒校验依赖本类，
     * 用户级吊销又需反向清理其 SSE 连接（先例 IamOAuth2AuthorizationRevocationService）
     */
    private final ObjectProvider<IamMessageSseService> messageSseServiceProvider;
    ...
    public int revokeAllByUserId(Long userId) {
        ...
        IamMessageSseService sseService = messageSseServiceProvider.getIfAvailable();
        if (sseService != null) {
