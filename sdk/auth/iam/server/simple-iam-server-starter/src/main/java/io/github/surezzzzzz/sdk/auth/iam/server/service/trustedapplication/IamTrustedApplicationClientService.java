...
public class IamTrustedApplicationClientService {
    ...
    private static final String SQL_FIND_APPLICATION_BY_REGISTERED_CLIENT =
    ...
    /**
     * 查询 OAuth2 注册客户端所属可信应用。
     *
     * <p>Consent 页面需要展示请求方的可信应用名称和图标。历史注册客户端可能没有
     * {@code application_id}，此时返回空，由调用方使用客户端名称与默认图标回退。
     *
     * @param registeredClientId SAS 注册客户端主键
     * @return 所属可信应用，历史客户端或已删除归属返回空
     */
    public Optional<IamTrustedApplicationEntity> findApplicationByRegisteredClientId(String registeredClientId) {
        List<Long> applicationIds = jdbcTemplate.queryForList(SQL_FIND_APPLICATION_BY_REGISTERED_CLIENT,
                Long.class, registeredClientId);
        if (applicationIds.isEmpty() || applicationIds.get(0) == null) {
            return Optional.empty();
        }
        return trustedApplicationRepository.findById(applicationIds.get(0));
    }
