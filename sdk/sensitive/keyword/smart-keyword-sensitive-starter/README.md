...
| 配置项 | 类型 | 默认值 | 说明 | 影响 |
|--------|------|--------|------|------|
| `retention-thresholds.none` | Double | 0.8 | 普通机构保留率阈值 | 保留率大于此值时触发降级，降低保留字段直到低于阈值 |
| `retention-thresholds.financial` | Double | 0.6 | 金融机构保留率阈值 | 金融机构更严格，保留率大于阈值即触发降级 |
| `retention-thresholds.government` | Double | 0.6 | 政府机构保留率阈值 | 政府机构更严格，保留率大于阈值即触发降级 |
| `retention-thresholds.education` | Double | 0.7 | 教育机构保留率阈值 | 教育机构中等严格，保留率大于阈值即触发降级 |
...
**机构类型判定**：
- 机构类型判定现在优先使用MetaInfo中的行业信息，避免仅依赖关键词匹配可能导致的误判
- 当MetaInfo中包含行业信息时，系统会根据行业信息直接判定机构类型
- 当MetaInfo中无行业信息时，系统会使用关键词匹配进行判定

**三级降级机制**：
1. **第一级降级**：保留率 > 阈值 → 关闭地域保留（keep-region=false）
2. **第二级降级**：保留率仍 > 阈值 → 关闭行业保留（keep-industry=false）
3. **第三级降级**：保留率仍 > 阈值 → 关闭组织类型保留（keep-org-type=false）
...
| `keyword-sets.financial-keywords` | Set<String> | 内置 | 金融机构敏感词 | 用于判定金融机构，触发对应保留率阈值                              |
| `keyword-sets.government-keywords` | Set<String> | 内置 | 政府机构敏感词 | 用于判定政府机构，触发对应保留率阈值                              |
| `keyword-sets.education-keywords` | Set<String> | 内置 | 教育机构敏感词 | 用于判定教育机构，触发对应保留率阈值                              |
...
当保留率大于阈值时，会依次关闭地域、行业、组织类型保留，直到保留率降到阈值以下。
