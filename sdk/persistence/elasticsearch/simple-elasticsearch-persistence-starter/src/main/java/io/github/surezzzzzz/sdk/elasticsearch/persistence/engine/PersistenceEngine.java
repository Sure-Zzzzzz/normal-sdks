...
public interface PersistenceEngine {
    ...
    /**
     * 先按 CREATE 新建，遇到 409 冲突后执行局部更新。
     */
    PersistenceResult createThenUpdateOnConflict(IndexRequest createRequest, UpdateRequest updateRequest);

    /**
     * 异步执行 create 冲突转 update。
     */
    CompletableFuture<PersistenceResult> createThenUpdateOnConflictAsync(IndexRequest createRequest,
                                                                         UpdateRequest updateRequest);

    /**
     * 批量先 CREATE，409 冲突 item 由 resolver 转成 UPDATE 后二次提交。
     */
    BulkResult bulkCreateThenUpdateOnConflict(BulkRequest createRequest, ConflictUpdateResolver resolver);

    /**
     * 异步批量执行 create 冲突转 update。
     */
    CompletableFuture<BulkResult> bulkCreateThenUpdateOnConflictAsync(BulkRequest createRequest,
