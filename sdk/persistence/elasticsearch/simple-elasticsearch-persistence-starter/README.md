...
```gradle
dependencies {
    implementation 'io.github.sure-zzzzzz:simple-elasticsearch-persistence-starter:1.1.1'
...
|---------|------|---------------|------|
| 1.1.1 | 1.0.3 | 1.2.1 | by-query 参数位置修复（body->URL）、scrollSize 优先、异步任务失败传播、新增 4 个 by-query 参数 |
...
- 直接升级 starter 依赖到 `simple-elasticsearch-persistence-starter:1.1.1`。
- starter 传递依赖 `simple-elasticsearch-persistence-core:1.0.3` 和 `simple-elasticsearch-route-starter:1.2.1`；如果工程显式声明了这两个依赖，请同步调整，避免 Helper / 模型 API 版本不一致。
- 原有 index / create / update / delete / bulk / byQuery 行为保持不变。
- by-query 执行参数（`refresh` / `slices` / `scroll_size` / `timeout` / `conflicts` / `wait_for_completion` 等）此前误写入 JSON body 触发 ES HTTP 400 的问题已修复，现在全部走 URL query。
- `scrollSize` 与 `batchSize` 同时配置时以 `scrollSize` 为准；只配置 `batchSize` 时作为 `scroll_size` 的兼容别名生效。
- 如果使用了异步 by-query 任务（`waitForCompletion(false)` + `getTask(...)`），注意失败任务现在会抛 `PersistenceExecutionException`，而非静默返回。
...
`updateByQuery` / `deleteByQuery` 的同步/异步都支持以下执行参数，1.1.1 起全部写入 URL query（不进 body）：

```java
ByQueryOptions.builder()
    .waitForCompletion(false)   // false=服务端异步返回 taskId，true/null=同步等待
    .conflicts("proceed")       // 冲突时继续
    .refresh(false)             // 完成后是否刷新
    .timeoutMs(30000L)          // 超时，映射 timeout=30000ms
    .slices(2)                  // 并行分片数
    .scrollSize(500)            // 映射 scroll_size；优先于 batchSize
    .batchSize(100)             // 兼容别名，仅 scrollSize 未配置时映射到 scroll_size
    .requestsPerSecond(500f)    // 节流，ES 6.x 忽略
    .maxDocs(1000L)             // 限制处理文档数，ES 6.x 忽略
    .waitForActiveShards(1)     // 活跃分片要求，ES 6.x 忽略
    .routing("test-route")      // 搜索阶段路由
    .build();
```

- `scrollSize` 优先于 `batchSize`；两者都为 null 时不透传 `scroll_size`。
- `query` 为 null 时默认 `match_all`，会匹配索引全部文档。异步 `update_by_query` / `delete_by_query` 误用会批量更新/删除全索引，调用方应显式传入限定范围的 query。
- 异步任务失败（任务不存在/已过期、任务执行失败、提交未返回 taskId）以 `PersistenceExecutionException` 抛出，不静默返回。
...
已从头重跑 `simple-elasticsearch-persistence-starter:test`（含 by-query 四条主链路 + 失败传播 + URL 参数单测）：

| Spring Boot | 工具链 | 目标 ES | 结果 |
|---|---|---|---|
| 2.7.9 | Gradle 8.5 + Java 11 | 7.17.16（9200） | 通过 |
| 2.4.5 | Gradle 7.6 + Java 8 + `spring.profiles.active=2.4.5` | 7.9.3（9204） | 通过 |
| 2.3.12 | Gradle 7.6 + Java 8 + `spring.profiles.active=2.3.12` | 7.9.3（9204） | 通过 |
| 2.2.x | Gradle 7.6 + Java 8 + `spring.profiles.active=2.2.x` | 6.2.2（9203） | 通过 |

多版本 profile 激活经测试报告实证确认（`The following profiles are active: persistence-test,<版本>,persistence-test-<版本>`），非假绿。`persistence-test` profile 下 primary = 9202（故意失败，测路由错误处理），secondary 为各版本对应 ES。
