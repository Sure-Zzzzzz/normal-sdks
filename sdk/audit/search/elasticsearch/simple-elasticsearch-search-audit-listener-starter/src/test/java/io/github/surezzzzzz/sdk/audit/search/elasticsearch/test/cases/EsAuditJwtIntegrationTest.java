 ...
 * ES审计 + JWT认证集成测试
 *
 * <p>测试真实的HTTP请求 -> JWT认证 -> ES查询 -> ES审计事件 -> 审计记录捕获的完整链路
 *
 * @author surezzzzzz
 * @since 1.0.0
 */
...
@SpringBootTest(
        ...
        properties = {
                ...
                "test.es.audit.provider-type=jwt"
        }
)
@AutoConfigureMockMvc
@ActiveProfiles("local")
public class EsAuditJwtIntegrationTest {

    private static final String TEST_INDEX = "audit_jwt_user";
    private static final String AGG_INDEX = "audit_jwt_order";
    private static final String API_INDEX_0 = "audit_jwt_log_0";
    private static final String API_INDEX_1 = "audit_jwt_log_1";
    ...
    @Test
    public void testJwtAuthenticationWithEsQuery() throws Exception {
        log.info("========== 测试：JWT认证 + ES查询 + 审计 ==========");
        ...
        log.info("✓ JWT查询审计通过: clientId={}, index={}, total={}",
                record.getClientId(), record.getIndexAlias(), record.getTotal());
    }

    @Test
    public void testJwtAuthenticationWithEsAgg() throws Exception {
        log.info("========== 测试：JWT认证 + ES聚合 + 审计 ==========");
        ...
        log.info("✓ JWT聚合审计通过: clientId={}, index={}", record.getClientId(), record.getIndexAlias());
    }

    @Test
    public void testMultipleEsQueriesWithJwt() throws Exception {
        log.info("========== 测试：JWT认证多个ES查询的审计 ==========");
        ...
        for (EsAuditRecord record : testEsAuditHandler.records) {
            assertNotNull(record.getClientId());
            assertTrue(record.getIndexAlias().startsWith("audit_jwt_log_"));
        }

        log.info("✓ JWT多查询审计通过: 收到 {} 条记录", testEsAuditHandler.records.size());
    }
