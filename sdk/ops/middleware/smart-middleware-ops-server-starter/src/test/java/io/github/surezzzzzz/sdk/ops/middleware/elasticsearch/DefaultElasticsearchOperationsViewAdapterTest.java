...
class DefaultElasticsearchOperationsViewAdapterTest {
    ...
    void shouldCancelFieldCapabilitiesRequestWhenDeadlineExpires() {
        ...
        DefaultElasticsearchOperationsViewAdapter timedOut = new DefaultElasticsearchOperationsViewAdapter(registry, 0L, 4096);
    ...
    @Test
    void shouldRejectOversizedRawSearchResponse() {
        RequestCapture capture = requestCapture("{\"hits\":{\"hits\":[]},\"padding\":\"" + repeat('x', 64) + "\"}");

        MiddlewareOpsException exception = assertThrows(MiddlewareOpsException.class,
                () -> capture.withMaxResponseLength(64).queryDocuments(documentRequest("{\"query\":{\"match_all\":{}}}")));
    ...
    private static class RequestCapture {
        ...
        private RequestCapture(SimpleElasticsearchRouteRegistry registry, AtomicReference<Request> request) {
            ...
            this.adapter = new DefaultElasticsearchOperationsViewAdapter(registry, 1000L, 4096);
        }

        private DefaultElasticsearchOperationsViewAdapter withMaxResponseLength(int maxResponseLength) {
            return new DefaultElasticsearchOperationsViewAdapter(registry, 1000L, maxResponseLength);
        }
    }
}
