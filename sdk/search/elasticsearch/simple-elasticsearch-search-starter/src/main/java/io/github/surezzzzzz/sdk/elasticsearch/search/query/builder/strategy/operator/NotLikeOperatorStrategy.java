...
/**
 * not_like 操作符策略（模糊不匹配）
 * 将 like 查询包装在 bool.must_not 中实现取反
 *
 * @author surezzzzzz
 */
@SimpleElasticsearchSearchComponent
@RequiredArgsConstructor
public class NotLikeOperatorStrategy implements OperatorStrategy {
    ...
    public QueryBuilder build(String fieldName, QueryCondition condition, FieldMetadata fieldMetadata) {
        return QueryBuilders.boolQuery().mustNot(
                likeStrategy.build(fieldName, condition, fieldMetadata));
    }
}
