...
| **通配符配置匹配具体索引** | **请求具体索引可匹配配置中的通配符 `name`，查询仍打到请求索引** | **v1.7.0+** |
...
| 1.1.2 | 写索引渲染能力抽取为 WriteIndexResolver 接口 |
| **1.2.0** | ES 兼容公共 Helper 下沉，供 search/persistence 直接复用（**当前推荐**） |
...
> route-starter 1.2.0 会自动传递依赖，无需手动添加。支持 Spring Boot 2.2.x / 2.3.12 / 2.4.5 / 2.7.9。
...
上例请求的是具体索引 `test_wildcard--2026.07.09`，会匹配配置中的 `name: "test_wildcard--*"`，实际查询仍打到请求索引，字段元数据、脱敏和日期配置使用匹配到的配置。
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `index` | String | 索引别名、配置名称或具体索引名（必填）；v1.7.0 起具体索引可匹配配置 `name` 中的通配符，`alias` 只支持精确匹配 |
...
框架按 `dateRange` 路由到日期分片索引。查询范围超大时（如全年 365 天）自动降级为月级通配符，避免 HTTP 请求行超限。

- 索引配置中必须设置 `date-split: true` 和 `date-pattern`
- 建议配置 `date-field`，框架自动追加时间过滤防止跨天脏数据
- 开启 `ignore-unavailable-indices: true` 可忽略不存在的索引

`dateRange` 只负责日期分片索引路由和日期字段过滤；业务过滤条件仍写在 `query` 中。如果只想按字段过滤，不要把业务字段放进 `dateRange`。

---

### 场景八：通配符配置匹配具体索引（v1.7.0+）
...
匹配顺序为：精确 `alias`、精确 `name`、按配置顺序匹配通配符 `name`。`alias` 不参与通配符匹配；多个通配符配置同时命中时，使用配置顺序靠前的一项并记录 warning。

---

### 场景九：通配索引防全量扫描（v1.4.0+）
...
### 场景十一：filter / filters 聚合对比分析（v1.5.6+）
...
### 场景十三：date_range / ip_range 聚合（v1.5.6+）
...
### 场景十四：Pipeline 聚合 Top N + HAVING（v1.5.5+）
...
### 场景十六：percentiles / percentile_ranks 聚合（v1.5.7+）
