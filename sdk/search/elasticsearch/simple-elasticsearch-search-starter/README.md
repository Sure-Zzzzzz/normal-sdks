...
| **NL 直接查询** | **直接用中文自然语言发起查询/聚合，无需构造 DSL** | **v1.6.1+** |
...
| + NL 转 DSL（`/api/nl/dsl`） | **1.6.1** | 1.0.10 | NL 翻译器 AND 条件 / 聚合名称 bug 在 1.6.1 修复 |
| + NL 直接查询（`/api/query/nl`） | **1.6.1** | 1.0.10 | 同上 |
...
### POST /api/query/nl — 自然语言直接查询（v1.6.1+）
...
| `dataSource` | String | 索引别名，优先级高于 NL 文本中的索引提示（可选） |
...
### POST /api/agg/nl — 自然语言直接聚合（v1.6.1+）
...
### 场景十七：自然语言直接查询（v1.6.1+）

无需构造 DSL，直接用中文描述查询意图。
...
- **NL 不走降级重试**：`/api/query/nl` 和 `/api/agg/nl` 不经过 QueryExecutor 的降级逻辑，日期分割索引的降级需自行处理
