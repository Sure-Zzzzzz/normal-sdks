...
| 文件 | 说明 |
|------|------|
| `README.md` | 模块说明、版本选型表、依赖、配置示例、升级指南 |
| `CHANGELOG.{版本}.md` | 每个版本一份，记录该版本变更 |
| `DESIGN.{版本}.md` | 设计文档（有设计决策的版本） |

**写作要求**：

1. **不写业务真实信息**：问题背景 / 示例 / 测试场景用 mock 场景描述，不写真实业务索引名、字段名。
   - ✅ 通配符索引用 `test_wildcard--*`（`test_wildcard--2024/2025/2026`），字段用 `extraField`/`extraField2` 等中性名
   - ❌ 不要写 `k01_device_ioc_external_compression--*`、`aptLevel`、`appVersion` 等真实业务名
2. **中性表述**："业务方反馈 / 业务方判断 / 业务场景 / 业务已确认"改成"初步判断 / 该场景 / 已确认"，"业务显式指定"改成"调用方显式指定"。
3. **测试代码 mock 也用中性名**：测试类的 mock 字段名、测试类名、注释、断言都用中性名（`extraField`），不要借用业务概念名。
4. **写完 grep 验证**：发布前 grep `业务方|真实业务索引名|真实业务字段名`，确保文档和测试代码干净。

**CHANGELOG.{版本}.md 内容结构**：

- 发布日期、类型（Bug Fix / Feature / 优化）
- 依赖升级表
- 变更内容（问题背景用 mock 场景 + 根因 + 修复方案 + 合并规则等）
- 新增测试（测试类名 + 说明，测试类名用中性名）
- 向后兼容性
- 升级指南
